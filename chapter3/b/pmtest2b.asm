...
.2:
	...
	pop esi
	ret
;做的事情就是将字符串ABCD...转换成对应的16进制数
DispAL:
	push ecx
	push edx
	mov ah,0ch   	;黑底红字
	mov dl,al 		;暂存al中的字符
	shr al,4 		;右移动4位，取得高4位
	mov ecx,2 		;循环计数
.begin:
	and al,01111b 	;只取al寄存器中值的低4位
	cmp al,9 		;和9比较大小
	ja .1 			;大于则转移
	add al,'0' 		;加“0”
	jmp .2
.1:
	...
	add al,'A'
.2:
	mov [gs:edi],ax   ;通过段描述符找到显存地址，然后在那个地方显示字符
	add edi,2
	mov al,dl
	loop .begin
	add edi,2   ;注意这里多了一个显存地址的偏移量多加2,这样会显示一个空格
	pop edx
	pop ecx
	ret
