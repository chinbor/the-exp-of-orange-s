...
;================================================================================================
;这里都是表示的引导扇区中的bs以及bpb域
	jmp short LABEL_START		; Start to boot.
	nop				; 这个 nop 不可少(构成3字节长度)

%include "fat12hdr.inc"			;里面包含了一些与fat12文件有关的内容（常量）
;接下来的都是引导扇区的引导代码，数据以及其他填充字符等（总共一起构成引导扇区的512字节）
LABEL_START:	
	mov	ax, cs
	mov	ds, ax
	mov	es, ax
	...
	mov	dh, 0			; "Booting  "
	call	DispStr			; 显示字符串Booting
	
	xor	ah, ah	; ┓
	xor	dl, dl	; ┣ 软驱复位，应该类似于初始化的工作，用于软驱的校准工作
	int	13h	; ┛
...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	mov	cl, 1           	;读一个扇区
	call	ReadSector      	;作用就是根据扇区号（逻辑）找出具体的在软盘中的柱面号，磁道号，扇区（物理），然后将内容读取到内存es：bx处
...
LABEL_DIFFERENT:
	and	di, 0FFE0h				; else ┓ di &= E0 为了让它指向本条目开头，因为一个条目32字节也就是100000
	add	di, 20h					;      ┃
	mov	si, LoaderFileName			;      ┣ di += 20h  下一个目录条目刚好就是加32
...
;变量
;----------------------------------------------------------------------------
wRootDirSizeForLoop	dw	RootDirSectors	; Root Directory 占用的扇区数, 在循环中会递减至零.值为14
wSectorNo		dw	0		; 要读取的扇区号
bOdd			db	0		; 奇数还是偶数，是为了根据根目录区中的文件条目的第26字节（文件起始簇号）寻找fatentry而准备
...
DispStr:
	...
	mov	ax, 01301h		; AH = 13h表示显示字符串,  AL = 01h表示写入方式光标跟随移动
	mov	bx, 0007h		; 页号为0(BH = 0) 黑底白字(BL = 07h)
	mov	dl, 0 			; dh代表起始行号，dl代表起始列号
	int	10h			; int 10h
	ret
...
GetFATEntry:
	...
	mul	bx			; dx:ax = ax * 3
	mov	bx, 2
	div	bx			; dx:ax / 2  ==>  ax <- 商, dx <- 余数，簇号*3/2的商值为fat中的偏移（0开始），dx是用来判断簇号是奇数还是偶数，他们两者的fatentry计算不一样，若为偶数，那么dx=0，若为奇数，那么dx=1
	cmp	dx, 0
	jz	LABEL_EVEN
	mov	byte [bOdd], 1
LABEL_EVEN:
	...
	mov	cl, 2
	call	ReadSector 		; 读取 FATEntry 的值所在数据区的扇区, 一次读两个, 避免在边界
...
LABEL_EVEN_2:
	and	ax, 0FFFh 		; 是为了与后面的FFFh作比较从而判断是否簇为文件结束
