...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	mov	cl, 1 			; 读一个扇区
	call	ReadSector		; 根据ax（扇区号）找出柱面号，磁道号，扇区从而将这一个扇区读到es:bx处

	mov	si, KernelFileName	; ds:si -> "KERNEL  BIN"
	mov	di, OffsetOfKernelFile
	cld
	mov	dx, 10h  		; 一个扇区16个条目，一个条目32字节
...
LABEL_FILENAME_FOUND:			; 找到 KERNEL.BIN 后便来到这里继续
	mov	ax, RootDirSectors 	; 14(根目录的扇区数)因为我们规定了根目录一共有224个条目
	and	di, 0FFF0h		; di -> 当前条目的开始，因为再我们逐个字节比较文件名的时候，di的值是依次加1的，到最后加了11次，所以需要进行这个操作从而到当前条目开始字节的位置

	push	eax
	mov	eax, [es : di + 01Ch]		; `.条目 的最后四个字节就是file size
	mov	dword [dwKernelSize], eax	; / 保存 KERNEL.BIN 文件大小
	pop	eax

	add	di, 01Ah		; di -> 首 Sector（就是此条目对应的开始簇号）
	mov	cx, word [es:di]
	push	cx			; 保存此 Sector 在 FAT 中的序号
	add	cx, ax
	add	cx, DeltaSectorNo	; cx <- KERNEL.BIN 的起始扇区号(0-based)
	mov	ax, BaseOfKernelFile
	mov	es, ax			; es <- BaseOfKernelFile
	mov	bx, OffsetOfKernelFile	; bx <- OffsetOfKernelFile
	mov	ax, cx			; ax <- Sector 号（kernel.bin的起始扇区号）

LABEL_GOON_LOADING_FILE:		; 将kernel.bin文件占用的所有扇区加载入内存
	push	ax			; `.
	push	bx			;  |
	...
	mov	cl, 1
	call	ReadSector
	pop	ax			; 取出此 Sector 在 FAT 中的序号，也就是前面push cx的内容
	call	GetFATEntry		; 根据ax（fat中的序号）找到fatentry并存放再ax中
	cmp	ax, 0FFFh 		; 对找到的fatentry进行判断，是否是文件的最后一个簇
	jz	LABEL_FILE_LOADED
	push	ax			; 保存 Sector 在 FAT 中的序号
	mov	dx, RootDirSectors
	add	ax, dx
	add	ax, DeltaSectorNo	; 扇区号
	add	bx, [BPB_BytsPerSec] 	; 因为读取了一个扇区到内存占用512字节，所以偏移量需要+512
