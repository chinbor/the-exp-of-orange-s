...
.Begin:
        ...
        cmp   eax, 0                      ; PT_NULL(说明无效，所以不需要这个program)
        jz    .NoAction
        push  dword [esi + 010h]    ;size ; p_filesz
        mov   eax, [esi + 04h]            ; p_offset
        add   eax, BaseOfKernelFilePhyAddr; 因为内核kernel.bin加载再80000h处，加上program header的第一个条目的第一个字节的偏移 
        push  eax		    ;src  ; 
        push  dword [esi + 08h]     ;dst  ; p_vaddr
        call  MemCpy                      ; 将src地址的内容复制到dst地址（30000h）处
        add   esp, 12                     ; 恢复esp
.NoAction:
        add   esi, 020h                   ; 每一个program的大小为20h（32字节）可以通过readelf -h 进行查看
        dec   ecx 		          ; 
        jnz   .Begin
