...
; =======================================================================================================================
restart:
	mov	esp, [p_proc_ready]					; p_proc_ready是一个结构体指针，表示的是这个结构体第一个元素的地址，然而这个结构体第一个元素是一个结构体，所以第一个元素的值也是一个地址，是所指向结构体第
											; 一个元素的地址,所以这里就指向了gs字段，也就是进程表的首地址
	lldt	[esp + P_LDT_SEL]				; 加载ldt再gdt中的选择子到ldtr寄存器中
	lea	eax, [esp + P_STACKTOP]				; esp+72的值给eax,此时指向的就是p_stacktop字段
	mov	dword [tss + TSS3_S_SP0], eax		; tss中的ring0堆栈指针esp等于这个地址，目的是为了当中断发生，也就是当特权级1到特权级0转移的时候就会到tss中取这个ring0的esp域，然后将当前进程的ss，esp，efl
											; ags，cs，eip，错误码（如果有的话）进栈，从而达到保存当前进程状态的作用
restart_reenter:
	dec dword [k_reenter]
	pop	gs
	pop	fs
