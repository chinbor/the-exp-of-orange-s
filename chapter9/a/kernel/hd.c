...
PRIVATE void init_hd()
{
	...
	printl("NrDrives:%d.\n", *pNrDrives);
	assert(*pNrDrives);
	/* 硬盘中断号为14,而且是再从片上的,这里是设置硬盘中断的处理程序 */
	put_irq_handler(AT_WINI_IRQ, hd_handler);
	/* 允许2号中断，也就是和从片相连的那个中断号 */
...
PRIVATE void hd_identify(int drive)
{
	...
	/* 向硬盘驱动器发送命令,这里可能出现两种情况，1是再调用完之后硬盘就再工作状态了，2是执行完这个hd_cmd_out函数调度程序调度了另一个进程执行 */
	hd_cmd_out(&cmd);
	/* 这里增加一个等待硬盘中断，当然是通过消息机制进行的 */
	interrupt_wait();
	/* 读取信息到hdbuf */
...
{
	MESSAGE msg;
	/* 对于INTURRUPT类型的消息，具体请见msg_receive函数,注意里面的has_int_msg，这个是在hd_handel（硬盘运行结束后会自动调用14号中断也就是这个中断处理程序）中设置的 */
